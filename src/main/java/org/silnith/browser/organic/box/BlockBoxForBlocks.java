...
public class BlockBoxForBlocks implements BlockLevelBox, BlockFormattingContext {
	...
	public ResolvedMarginInformation getMarginInformation(
			final AbsoluteLength containingBlockWidth) {
	...
	public AbsoluteLength negotiateWidth(final AbsoluteLength containingBlockWidth, final AbsoluteLength targetWidth, final Graphics2D graphics) {
		...
		for (final BlockLevelBox child : children) {
			...
			final AbsoluteLength childNegotiatedWidth = child.negotiateWidth(childContainingBlockWidth, childMinusMargins, graphics);
...
//		final float paddingRight = getPixels(resolvedPaddingInformation.getPaddingRight());
		...
		for (final BlockLevelBox child : children) {
			...
			final RenderableContent renderableContent = child.layoutContents(childContainingBlock, childMinusMargins, graphics);
			childRenderables.add(renderableContent);
			final Dimension2D childSize = renderableContent.getSize();
			currentChildLocation.setLocation(currentChildLocation.getX(),
					currentChildLocation.getY() + getPixels(marginBetweenBlocks) + childSize.getHeight() + getPixels(marginBottom));
			// bottom margin of child
			contentSize.setSize(Math.max(contentSize.getWidth(), getPixels(marginLeft) + childSize.getWidth() + getPixels(marginRight)),
					getPixels(marginBetweenBlocks) + contentSize.getHeight() + childSize.getHeight() + getPixels(marginBottom));
			
			previousBottomMargin = marginBottom;
		}
		
		/*
		 * TODO: Should the block expand horizontally to fill all available space?
		 */
...
//		final float totalWidth = borderLeft + paddedContentWidth + borderRight;
		...
		final RenderedBox box = new RenderedBox(
				paddedContentWidth, paddedContentHeight,
				borderInformation);

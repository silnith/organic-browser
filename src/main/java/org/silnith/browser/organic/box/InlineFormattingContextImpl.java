...
public class InlineFormattingContextImpl implements InlineFormattingContext, BlockLevelBox {
    ...
    public ResolvedMarginInformation getMarginInformation(final AbsoluteLength parentWidth) {
        return new ResolvedMarginInformation(AbsoluteLength.ZERO, AbsoluteLength.ZERO, AbsoluteLength.ZERO,
                AbsoluteLength.ZERO);
    }
    ...
    public AbsoluteLength negotiateWidth(final AbsoluteLength containingBlockWidth, final AbsoluteLength targetWidth,
        ...
        for (final InlineLevelBox child : children) {
            final AbsoluteLength childNegotiatedWidth =
                    child.negotiateWidth(containingBlockWidth, targetWidth, graphics);
    ...
    private void createNewLineBox(final List<RenderableLineContent> lineContent, final Dimension2D contentSize,
            final List<LineBox> lineBoxes) {
        assert !lineContent.isEmpty();
        
        final LineBox lineBox = new LineBox(lineContent);
        final Dimension2D lineBoxSize = lineBox.getSize();
        contentSize.setSize(Math.max(contentSize.getWidth(), lineBoxSize.getWidth()),
                contentSize.getHeight() + lineBoxSize.getHeight());
    ...
    @Override
    public synchronized RenderableContent layoutContents(final AbsoluteLength containingBlockWidth,
        ...
        boolean done = false;
        while ( !done) {
            final InlineLevelBox.LayoutResults initialResults =
                    currentChild.layoutContents(containingBlockWidth, remainingWidth, graphics, lineContents.isEmpty());
            ...
            if (initialResults == null) {
                ...
                results = currentChild.layoutContents(containingBlockWidth, remainingWidth, graphics,

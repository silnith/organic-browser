 ...
 * 
 * <p>This class is thread-safe and re-entrant.
 * 
 * @author kent
 */
public class Formatter {
	...
	public BlockLevelBox createListItemBox(final StyledElement styledElement) {
		...
		final ListStylePosition listStylePosition = listStylePositionAccessor.getComputedValue(styledElement.getStyleData());
		if (requiresBlockFormattingContext(styledElement)) {
			...
			case INSIDE: {
				...
				// do createInlineLevelBox with an inline child appended to the front for the marker
			...
			case OUTSIDE: {
				// make block box for marker and anonymous block box for list item 
				final BlockLevelBox markerBox = new BlockListItemMarker(fontSizeAccessor, styledElement);
	...
	private void fillInBlockChildren(final StyledElement styledBlockElement,
			final BlockFormattingContext box, final List<StyledContent> children) {
	...
	private void fillInInlineChildren(final InlineFormattingContext box,
	...
	private boolean isInlineLevel(final Display display) {
		...
		case INLINE_TABLE: return true;
		case INLINE_BLOCK: return true;
		default: return false;
		}
	}

 ...
 * 
 * <p>This class is thread-safe and re-entrant.  However, the {@link StyledElement}
 * passed to it may not be.  A styled element must be used by at most one
 * cascade applier, but a cascade applier may be run on as many styled elements
 * as desired.
 * 
 * @author kent
 */
public class CascadeApplier {
	...
	public void cascade(final StyledElement styledElement, final Collection<CSSRule> rules, final Collection<CSSPseudoElementRule> pseudoRules) {
...
//				compute(child);
	...
	public void compute(final StyledContent styledContent) {
		...
		// run through all properties and set the computed value from the specified value

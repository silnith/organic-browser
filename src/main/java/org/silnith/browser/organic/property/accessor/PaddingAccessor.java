...
public abstract class PaddingAccessor extends PropertyAccessor<Length<?>> {
	...
	public PaddingAccessor(final PropertyName propertyName,
	...
	protected Length<?> parse(final StyleData styleData, final String specifiedValue) {
		...
		case PERCENTAGE: {
			...
			if (percentageLength.getLength().floatValue() < 0) {
				throw new IllegalArgumentException("Padding values cannot be negative: " + getPropertyName() + ": " + percentageLength);
			}
		...
		if (absoluteLength.getLength().floatValue() < 0) {
			throw new IllegalArgumentException("Padding values cannot be negative: " + getPropertyName() + ": " + absoluteLength);
		}

...
public class HTML4Parser implements FileParser<Document> {
	...
	public Document parse(final Download download) throws IOException {
		...
		// else, error
		while (token != null
				&& token.getType() != Token.Type.DOCTYPE) {
	...
	public Document parse2(final Download download) throws IOException {
		final Reader characterIterator = new InputStreamReader(download.getInputStream(), download.getContentEncoding());
		...
		if (nextToken == Token.Type.START_TAG) {
			// read HTML
			StartTag startTag = tokenizer.readStartTag();
	...
	public static void main(final String[] args) throws IOException {
		...
		download.connect();
		download.download();
		HTML4Parser parser = new HTML4Parser();

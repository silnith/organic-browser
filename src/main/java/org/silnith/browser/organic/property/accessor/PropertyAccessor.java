...
/**
 * The base class for all CSS property accessors.
 * This contains methods to compute the value from the specified value, and to
 * handle inheritance and default values.  Each property accessor will subclass
 * this and provide implementations for parsing specified values into the type
 * {@code &lt;T&gt;} as well as providing initial values.
 * 
 * <p>Property accessors must all be thread-safe and re-entrant.
 * 
...
public abstract class PropertyAccessor<T> {
	...
	/**
	 * Constructs a new {@code PropertyAccessor} with the given {@code propertyName}
	 * and value for whether the property is inherited by default.
	 * Note that a stylesheet may specify any particular property as inherited
	 * for a specific element, the {@code inherited} value here is simply what
	 * the default is for unspecified properties.
	 * 
	...
	/**
	 * Returns the computed value of the parent style of {@code styleData}.
	 * If {@code styleData} is the root style, then this returns
	 ...
	 * 
	 * @param styleData a few properties are defined in terms of other properties
	 *         on the same element (e.g. border-color)
	 ...
	 *       string "inherit", then the property value is the computed value of
	 *       the same property for the parent element.</li>
	 *   <li>If the property is specified explicitly for this element and is not
	 *       the string "inherit", {@link #parse} is
	 *       called to convert the string value into the type {@code &lt;T&gt;}.</li>
	 *   <li>If the property is not specified for this element but is inherited
	 *       by default, then the property value is the computed value of the
	 *       same property for the parent element.</li>
	 *   <li>If the property is not specified for this element and is not
	 *       inherited by default, then {@link #getInitialValue}
	 *       is called to get the computed value.</li>
	 * </ol>
	 * <p>For inherited values, if the element is the root element then the value
...
//		assert !styleData.isPropertyComputed(propertyName);
	 ...
	 * {@code null}.
	 * 
	 * @return a set containing the names of all properties that must be computed
	 *         before this property can be computed
	 */

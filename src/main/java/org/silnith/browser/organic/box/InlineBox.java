...
public class InlineBox implements InlineLevelBox, InlineFormattingContext {
    ...
    private class Continuation implements InlineLevelBox {
        ...
        private final InlineLevelBox currentChild;
        
        private final Iterator<InlineLevelBox> iterator;
        
        private final ResolvedPaddingInformation resolvedPaddingInformation;
        
        public Continuation(final InlineLevelBox currentChild, final Iterator<InlineLevelBox> iterator,
        ...
        public LayoutResults layoutContents(final AbsoluteLength containingBlockWidth, final AbsoluteLength targetWidth,
            ...
            return fillLine(containingBlockWidth, childTargetWidth, graphics, canOverflow, resolvedPaddingInformation,
                    currentChild, iterator, true);
        }
    ...
    public AbsoluteLength negotiateWidth(final AbsoluteLength containingBlockWidth, final AbsoluteLength targetWidth,
        ...
        for (final InlineLevelBox child : children) {
            final AbsoluteLength childNegotiatedWidth =
                    child.negotiateWidth(containingBlockWidth, childTargetWidth, graphics);
    ...
    public LayoutResults layoutContents(final AbsoluteLength containingBlockWidth, final AbsoluteLength targetWidth,
        ...
        if (children.isEmpty()) {
            ...
            final WrappedLineBox wrapped = new WrappedLineBox(noChildren, false, false, backgroundInformation,
                    borderInformation, resolvedPaddingInformation);
            return new Results(wrapped);
        }
        
        final Iterator<InlineLevelBox> iterator = children.iterator();
        final InlineLevelBox currentChild = iterator.next();
        
        return fillLine(containingBlockWidth, childTargetWidth, graphics, canOverflow, resolvedPaddingInformation,
                currentChild, iterator, false);
    }
    
    private LayoutResults fillLine(final AbsoluteLength containingBlockWidth, AbsoluteLength remainingWidth,
            final Graphics2D graphics, final boolean canOverflow,
            final ResolvedPaddingInformation resolvedPaddingInformation, InlineLevelBox currentChild,
        ...
        while ( !done) {
            final LayoutResults childLayout = currentChild.layoutContents(containingBlockWidth, remainingWidth,
                    graphics, canOverflow && renderables.isEmpty());
            ...
            final AbsoluteLength childWidth =
                    new AbsoluteLength((float) childRenderable.getSize().getWidth(), AbsoluteUnit.PX);
        ...
        if (renderables.isEmpty()) {
            ...
            if (currentChild == null) {
                final WrappedLineBox wrappedLineBox = new WrappedLineBox(renderables, openAtStart, false,
                        backgroundInformation, borderInformation, resolvedPaddingInformation);
                return new Results(wrappedLineBox);
            } else {
                final WrappedLineBox wrappedLineBox = new WrappedLineBox(renderables, openAtStart, true,
                        backgroundInformation, borderInformation, resolvedPaddingInformation);
                return new Results(wrappedLineBox,
                        new Continuation(currentChild, iterator, resolvedPaddingInformation));
            }
        }
    }

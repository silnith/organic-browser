...
public class PaddingInformation {
	...
	private static final PropertyAccessor<Length<?>> paddingTopAccessor = (PropertyAccessor<Length<?>>) propertyAccessorFactory.getPropertyAccessor(PropertyName.PADDING_TOP);
	private static final PropertyAccessor<Length<?>> paddingRightAccessor = (PropertyAccessor<Length<?>>) propertyAccessorFactory.getPropertyAccessor(PropertyName.PADDING_RIGHT);
	private static final PropertyAccessor<Length<?>> paddingBottomAccessor = (PropertyAccessor<Length<?>>) propertyAccessorFactory.getPropertyAccessor(PropertyName.PADDING_BOTTOM);
	private static final PropertyAccessor<Length<?>> paddingLeftAccessor = (PropertyAccessor<Length<?>>) propertyAccessorFactory.getPropertyAccessor(PropertyName.PADDING_LEFT);
	...
	public PaddingInformation(final Length<?> paddingTop, final Length<?> paddingRight, final Length<?> paddingBottom, final Length<?> paddingLeft) {
		super();
		if (paddingTop == null) {
...
//			final RelativeLength relativeLength = (RelativeLength) length;
			throw new IllegalStateException("Property should have been resolved into an absolute length or percentage before layout: " + length);
		} // break;
		case PERCENTAGE: {
